...
for (n in c('foreign', 'utils', 'relimp', 'ggplot2', 'ggdendro', 
            'dplyr', 'tidyr', 'reshape2', 'tibble', 'captioner',
           'psych', 'memisc', 'Hmisc', 'ltm', 
...
- Does the parallel analysis propose a number of factors consistent with Step 3?
...
# for ordinal - hierarhical clustering with complete linkage on distance matrix with Euclidean distances (default)
hclust.mydata <- hclust(dist(mydata), "complete")
# or for binary - hierarhical clustering with complete linkage on distance matrix for binary data (Jaccard index)
...
```{r, include=FALSE, warning=FALSE, echo=FALSE, message=FALSE}
# save the cluster membership in the original dataset - for example if the data indicate a 3-cluster solution
mydata$clusternumber <- cutree(hclust.mydata, 3)
# percentages of participants in groups
#length(mydata$clusternumber[mydata$clusternumber==1])*100/length(mydata$clusternumber)
#length(mydata$clusternumber[mydata$clusternumber==2])*100/length(mydata$clusternumber)
#length(mydata$clusternumber[mydata$clusternumber==3])*100/length(mydata$clusternumber)
# compare cluster groups on items
for( n in myitems)
{
  cat( "\n", n, ":" );
  print( chisq.test(mydata$clusternumber, mydata[,n]) );
}
# compare cluster groups on e.g. demographics
# (***** make sure you add your demographic to the dataset)
# t.test(clusternumber ~ mydata$GENDER, mydata)
```

```{r, fig.width=10, fig.height=5, warning=FALSE, echo=FALSE, message=FALSE, fig.cap=Fig_5.2_cap}
# visualize the clusters
data1 <- mydata %>% 
  gather(item,score,-clusternumber) %>% 
  group_by(item) %>% 
  mutate(rescaledscore=(score-min(score))/(max(score)-min(score)))

data1$clusternumber <- as.factor(data1$clusternumber)

# flat means plot if needed
# m <- ggplot(data1, aes(y = score, x = item,
#     fill=clusternumber))
# m + theme(panel.background = element_rect(fill='white', colour='black'), 
#           panel.grid.major= element_line(colour='lightgrey'), 
#           legend.key=element_rect(fill="white"), 
#           axis.text.x = element_text(angle = 60, hjust = 1))  + 
#   labs(fill = "Cluster number", colour = "Cluster number") +  
#   geom_point(stat = "summary", fun.y = "mean", aes(colour=clusternumber) )
...
# plot
ggplot(data1, aes(x = item, y = score)) +
  geom_polygon(stat = "summary", fun.y = "mean", aes(group = clusternumber, color = clusternumber), fill = NA, size = 2, show.legend = FALSE) +
  geom_line(stat = "summary", fun.y = "mean", aes(group = clusternumber, color = clusternumber), size = 2) +
  ...
  xlab("") + ylab("") + labs(fill = "Cluster number", colour = "Cluster number")  +
  guides(color = guide_legend(ncol=2)) +
  coord_radar()

```

The resulting clusters have the following percentages of the total sample:

- cluster 1: `r round(length(mydata$clusternumber[mydata$clusternumber==1])*100/length(mydata$clusternumber), 2)`%
- cluster 2: `r round(length(mydata$clusternumber[mydata$clusternumber==2])*100/length(mydata$clusternumber), 2)`%
- cluster 3: `r round(length(mydata$clusternumber[mydata$clusternumber==3])*100/length(mydata$clusternumber), 2)`%

```{r, include=FALSE, warning=FALSE, echo=FALSE, message=FALSE}
# and/or perform 

# for ordinal data: k-means can be used (more commonly used)
kmmydata <- kmeans(mydata, 3)
kmdata <- data.frame(mydata[, -ncol(mydata)], K=kmmydata$cluster) #Include the new number of cluster in the data set

# for binary data: k-medoids (more robust than k-means)
# pammydata <- pam(dist(mydata, method="binary"), 3) # computed from the distance matrix, can be computed from dataset too: pammydata <- pam(mydata, 3)

```

```{r, fig.width=5, fig.height=5, warning=FALSE, echo=FALSE, message=FALSE, fig.cap=Fig_5.3_cap}
# plot the clusters (should be distinct groups)
clusplot(mydata, kmmydata$cluster, color=TRUE, shade=TRUE, 
                  labels=1, lines=0)

# or for k-medoids clusters
# clusplot(pammydata)
# consider plotting also the silhouette values (good if close to 1, bad if close to 0, more appropriate for the neighbouring cluster if negative value)
# plot(pammydata)
# and the average silhouette width for reporting: (indicates reasonable cluster classification if >.5, no cluster structure if <.2)
# pammydata$silinfo$avg.width

```

```{r, include=FALSE, warning=FALSE, echo=FALSE, message=FALSE}
# amazing 3d plot
pcdf <- princomp(mydata[, -ncol(mydata)],cor=T,score=T)
summary(pcdf) # Compute the validity of each component/dimension
```

```{r, webgl=TRUE, fig.width=5, fig.height=5, warning=FALSE, echo=FALSE, message=FALSE, fig.cap=Fig_5.4_cap}
plot3d(pcdf$scores, col=rainbow(3)[kmdata$K], main = "Cluster location on first 3 principal components")#Create a 3D plot
legend3d("topright", legend = paste('Cluster number', c('1', '2', '3')), pch = 15, col = rainbow(3), cex=0.80)

```

## Interpretation

- Can item responses (also/better) be characterized as categories of respondents?
- if yes, 
  - what do the groups represent in terms of item content?
  - how separate are these groups in terms of the variance in item responses?  
