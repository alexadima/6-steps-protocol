---
title: "Psychometric analyses Sickness Impact Profile Roland Scale: 6-steps protocol"
...
# Step 3: Item properties - parametric Item Response Theory (Rasch Model or Rating Scale Model)
...
tetramatrix <- tetrachoric(mydata[,1:24])
Calphamysubscale1 <- psych::alpha(tetramatrix$rho) 
...
omegamysubscale1 <- ci.reliability(data=mydata[,1:24], type="categorical", conf.level = 0.95,
...
tetramatrix2 <- tetrachoric(mysubscale2)
Calphamysubscale2 <- psych::alpha(tetramatrix2$rho) 
...
omegamysubscale2 <- ci.reliability(data=mysubscale2, type="categorical", conf.level = 0.95,
                                   interval.type="perc", B=10)
```
...
Beta = c(round(iclust(tetramatrix$rho)$beta, 2),
         round(iclust(tetramatrix2$rho)$beta, 2));
